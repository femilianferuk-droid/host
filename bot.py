...
def get_main_keyboard():
    ...
    builder.row(
        KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['upload']}'>📁</tg-emoji> Загрузить"),
    ...
    if ADMIN_IDS:
        builder.row(
            KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['settings']}'>⚙</tg-emoji> Админ панель")
        )
...
"""
    ...
    await callback.message.answer(
        "<tg-emoji emoji-id='{EMOJI[\"settings\"]}'>⚙</tg-emoji> Админ панель:",
        reply_markup=get_admin_keyboard()
    )
    await callback.answer()
...
async def cancel_media(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(
        "<tg-emoji emoji-id='{EMOJI[\"cross\"]}'>❌</tg-emoji> Установка медиа отменена."
    )
    await callback.message.answer(
        "<tg-emoji emoji-id='{EMOJI[\"settings\"]}'>⚙</tg-emoji> Админ панель:",
        reply_markup=get_admin_keyboard()
