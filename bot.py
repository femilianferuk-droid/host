...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        for s in sections:
            await conn.execute("""
                INSERT INTO media_settings (section) VALUES ($1) 
...
async def get_servers(user_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetch("""
            SELECT * FROM servers WHERE user_id = $1 AND status IN ('pending', 'active', 'pending_payment_done')
...
def back_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=f"◁ Назад")]],
        resize_keyboard=True
    )
...
def admin_menu_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=f"{em(EMOJI['media'])} Медиа")],
            [KeyboardButton(text=f"{em(EMOJI['stats'])} Статистика")],
...
async def cmd_start(message: Message):
    await create_user(message.from_user.id, message.from_user.username, message.from_user.full_name or "Пользователь")
...
async def profile(message: Message):
    ...
    pending_count = sum(1 for s in servers if s["status"] in ["pending", "pending_payment_done"])
...
@dp.message(Command("skip"), AdminMediaStates.waiting_media_file)
async def skip_media_caption(message: Message, state: FSMContext):
    pass  # Обрабатывается в receive_media_file
...
async def admin_stats(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        total_revenue = await conn.fetchval("SELECT COALESCE(SUM(price), 0) FROM servers WHERE status IN ('active', 'pending', 'pending_payment_done')")
