...
async def create_bot(user_id: int, data: dict) -> int:
    async with get_db() as conn:
        bot_record = await conn.fetchrow('''
            INSERT INTO bots (user_id, github_url, env_vars, bot_token, tariff_period, tariff_price, status)
            VALUES ($1, $2, $3, $4, $5, $6, 'pending')
            RETURNING id
        ''', user_id, data['github_url'], data['env_vars'], data['bot_token'], 
            data['tariff_period'], data['tariff_price'])
...
async def get_invoice_by_bot_id(bot_id: int) -> Optional[dict]:
    async with get_db() as conn:
        invoice = await conn.fetchrow('''
            SELECT * FROM invoices WHERE bot_id = $1 ORDER BY created_at DESC LIMIT 1
        ''', bot_id)
...
def get_main_keyboard() -> ReplyKeyboardMarkup:
    ...
    builder.row(
        KeyboardButton(text=f'<tg-emoji emoji-id="{EMOJI["download"]}"> </tg-emoji>Загрузить'),
        KeyboardButton(text=f'<tg-emoji emoji-id="{EMOJI["box"]}"> </tg-emoji>Мои сервера')
    )
    builder.row(
        KeyboardButton(text=f'<tg-emoji emoji-id="{EMOJI["profile"]}"> </tg-emoji>Профиль')
    )
...
def get_admin_keyboard() -> ReplyKeyboardMarkup:
    ...
    builder.row(
        KeyboardButton(text=f'<tg-emoji emoji-id="{EMOJI["profile"]}"> </tg-emoji>Профиль'),
        KeyboardButton(text=f'<tg-emoji emoji-id="{EMOJI["gear"]}"> </tg-emoji>Админ панель')
    )
    return builder.as_markup(resize_keyboard=True)
...
@router.message(F.text.contains("Загрузить"))
...
@router.message(F.text.contains("Мои сервера"))
